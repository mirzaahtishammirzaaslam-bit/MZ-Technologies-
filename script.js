...
setTimeout(() => {
  ...
  if (!prefersReducedMotion) {
    leadInterval = setInterval(() => {
      if (!document.hidden) injectLead();
    }, 3800);
  }
}, 3000);

// Pause lead updates when app is hidden (mobile optimization)
document.addEventListener('visibilitychange', () => {
  if (document.hidden) {
    if (leadInterval) clearInterval(leadInterval);
  } else if (!prefersReducedMotion && !leadInterval) {
    leadInterval = setInterval(() => {
      if (!document.hidden) injectLead();
    }, 3800);
  }
...
(function initTheme() {
  ...
  function initializeTheme() {
    ...
    const theme = savedTheme === 'light' ? 'light' : savedTheme === 'dark' ? 'dark' : (prefersDark ? 'dark' : 'light');
  ...
  function setupThemeToggle() {
    ...
    themeToggleBtn.addEventListener('click', () => {
      ...
      // Dispatch event for other scripts that might listen for theme changes
      window.dispatchEvent(new CustomEvent('themeChange', { detail: { theme: newTheme } }));
    });

    // Keyboard support for theme toggle (Space and Enter)
